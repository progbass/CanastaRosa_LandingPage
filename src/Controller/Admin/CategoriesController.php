...
class CategoriesController extends AppController
{
    ...
    public function image() {
        ...
        if ($this->request->is('post')) {
            ...
            $image = $imageTable->newEntity();
            $image->images = "/img/" . $name . $img['name'];
            $image->user_id = $this->Auth->user('id');
            $image->status = 0;
            $image->uniqueid = session_id();
            if ($imageTable->save($image)) {
                $this->request->session()->write("uniqueid", $image->uniqueid);
                $this->set("res", array(
                    'r' => 1,
                    'id'=>$image->id,
                    'img'=>$image->images
    ...
    /*
     * User View own Project list
     */
    
    public function viewproduct(){
        $product=  \Cake\ORM\TableRegistry::get('Products');
        $pro=$product->find("all")->contain(['Images'])->where(['user_id'=>$this->Auth->user('id')])->toArray();
       // debug($pro); exit;
